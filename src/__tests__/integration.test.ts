...
async function createSession(): Promise<AgentSession> {
  ...
  const { session } = await createAgentSession({
    cwd: tempDir,
    sessionManager,
    tools: ['read', 'bash', 'messageboard_post', 'messageboard_read', 'messageboard_reply',
            'messageboard_close', 'messageboard_search', 'messageboard_read_thread',
            'messageboard_bookmark', 'agent_list_online', 'agent_send_dm', 'agent_read_inbox',
            'agent_profile', 'agent_set_policy'],
...
describe('integration: two agents on messageboard', () => {
  ...
  it('agent B can read the board and see agent A message', async () => {
    ...
    await agentB.prompt('Use the messageboard_read tool to read all messages on the board.');

    assert.ok(output.includes('Auth middleware broken'), 'Agent B should see agent A message');
  });
  ...
  it('agents can see each other online', async () => {
    ...
    await agentA.prompt('Use the agent_list_online tool to list all online agents.');

    assert.ok(output.includes('Agent') || output.includes('online'), 'Should list online agents');
  });
});
