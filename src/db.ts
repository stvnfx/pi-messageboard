...
import type { Agent, Message, Reply, DirectMessage, Category, MessageStatus, InboxPolicy } from './types.js';
...
export function registerAgent(sessionId: string, name: string, suffix: string): Agent {
  const d = getDb();
...
export function getAgent(id: string): Agent | null {
  const d = getDb();
  const row = d.prepare('SELECT * FROM agents WHERE id = ?').get(id) as Agent | undefined;
  return row ?? null;
}
...
export function getOnlineAgents(): Agent[] {
  ...
  d.prepare('UPDATE agents SET status = ? WHERE status = ? AND last_heartbeat < ?')
    .run('offline', 'online', twoMinAgo);
  return d.prepare('SELECT * FROM agents WHERE status = ? ORDER BY name')
    .all('online') as Agent[];
}

export function updateHeartbeat(agentId: string): void {
  const d = getDb();
  d.prepare('UPDATE agents SET last_heartbeat = ?, status = ? WHERE id = ?')
    .run(Date.now(), 'online', agentId);
}

export function setAgentOffline(agentId: string): void {
  const d = getDb();
  d.prepare('UPDATE agents SET status = ? WHERE id = ?')
    .run('offline', agentId);
}

export function updateAgentInboxPolicy(agentId: string, policy: InboxPolicy): void {
  const d = getDb();
  d.prepare('UPDATE agents SET inbox_policy = ? WHERE id = ?')
    .run(policy, agentId);
}

// ─── Message Operations ─────────────────────────────────────────────

export function createMessage(author: string, category: Category, subject: string, body: string, tags: string[] = [], assignedTo?: string): Message {
  const d = getDb();
  const id = randomUUID();
  const now = Date.now();
  d.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, author, now, category, subject, body, JSON.stringify(tags), 'open', assignedTo ?? null);
...
export function getMessages(opts: { category?: Category; status?: MessageStatus; author?: string; tag?: string; assignedTo?: string; limit?: number } = {}): Message[] {
  ...
  if (opts.assignedTo) { conditions.push('assigned_to = ?'); params.push(opts.assignedTo); }
...
export function assignMessage(id: string, assignedTo: string): void {
  const d = getDb();
  d.prepare('UPDATE messages SET assigned_to = ?, status = ? WHERE id = ?')
    .run(assignedTo, 'claimed', id);
}
...
export function getReply(id: string): Reply | null {
  const d = getDb();
  return (d.prepare('SELECT * FROM replies WHERE id = ?').get(id) as Reply) ?? null;
}
...
export function sendDirectMessage(fromAgent: string, toAgent: string, subject: string, body: string): DirectMessage {
  const d = getDb();
  const id = randomUUID();
  const now = Date.now();
  d.prepare('INSERT INTO inbox (id, from_agent, to_agent, timestamp, subject, body, read) VALUES (?, ?, ?, ?, ?, ?, 0)')
...
export function getDirectMessage(id: string): DirectMessage | null {
  const d = getDb();
  return (d.prepare('SELECT * FROM inbox WHERE id = ?').get(id) as DirectMessage) ?? null;
}

export function getInbox(agentId: string, unreadOnly = false): DirectMessage[] {
  const d = getDb();
  const where = unreadOnly ? 'WHERE to_agent = ? AND read = 0' : 'WHERE to_agent = ?';
  return d.prepare(`SELECT * FROM inbox ${where} ORDER BY timestamp DESC LIMIT 50`)
...
export function addBookmark(agentId: string, messageId: string): void {
  const d = getDb();
  d.prepare('INSERT OR IGNORE INTO bookmarks (agent_id, message_id, timestamp) VALUES (?, ?, ?)')
...
export function removeBookmark(agentId: string, messageId: string): void {
  const d = getDb();
  d.prepare('DELETE FROM bookmarks WHERE agent_id = ? AND message_id = ?').run(agentId, messageId);
}
