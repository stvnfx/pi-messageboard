...
export function registerSpawnTools(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description: 'Spawn a new agent with a mythology name that appears on the messageboard',
    ...
    parameters: Type.Object({
      task: Type.Optional(Type.String({ description: 'Task to assign to the new agent' })),
      model: Type.Optional(Type.String({ description: 'Model for the agent (e.g. anthropic/claude-sonnet-4-5)' })),
      parent_agent: Type.Optional(Type.String({ description: 'Parent agent ID (auto-detected if omitted)' })),
    }),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      body: Type.String({ description: 'Task description (supports markdown)' }),
    }),
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      const myId = getMyAgentId(ctx);
      const agent = mbDb.getMbAgent(params.agent_id);
      if (!agent) {
        return { content: [{ type: 'text', text: `Agent "${params.agent_id}" not found.` }], details: {}, isError: true };
      }
      ...
      if (agent.status === 'online' || agent.status === 'busy') {
        ctx.ui.notify(`📋 Task assigned to ${params.agent_id}: ${params.subject}`, 'info');
      }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({}),
    async execute(_toolCallId: string, _params: Record<string, unknown>, _signal: AbortSignal, _onUpdate?: unknown, _ctx?: unknown) {
      ...
      const lines = [
        ...
        ``,
        `Online Agents:`,
        ...online.map(a => `  ${a.id} ${a.status === 'busy' ? '🔄' : '✅'}${a.task ? ` — ${a.task}` : ''}`),
        ``,
        `Active Loops: ${loops.length}`,
        ...loops.map(l => `  [${l.id.slice(0, 8)}] ${l.status} iter ${l.iteration}: ${l.goal.slice(0, 50)}`),
        ``,
        `Recent Board Activity:`,
        ...recentMessages.map(m => `  [${m.id.slice(0, 8)}] ${m.category.toUpperCase()}: ${m.subject} — ${m.author}`),
      ];

      return {
        content: [{ type: 'text', text: lines.join('\n') }],
        details: { agentCount: agents.length, onlineCount: online.length, loopCount: loops.length },
      };
    },
  });
}
