...
export function registerMbAgent(
	agent: Omit<MbAgent, "spawn_time"> & { spawn_time?: number },
): MbAgent {
	const d = getDb();
	const now = Date.now();
  ...
  `).run(
		...
		full.task ?? null,
		full.task_post_id ?? null,
		full.spawned_by ?? null,
		full.spawn_time,
		full.loop_id ?? null,
	);
...
export function getAllMbAgents(): MbAgent[] {
	const d = getDb();
	return d
		.prepare("SELECT * FROM mb_agents ORDER BY spawn_time DESC")
		.all() as MbAgent[];
}
...
export function getOnlineMbAgents(): MbAgent[] {
	...
	d.prepare(
		"UPDATE mb_agents SET status = ? WHERE status = ? AND last_heartbeat < ?",
	).run("offline", "online", twoMinAgo);
	return d
		.prepare("SELECT * FROM mb_agents WHERE status IN (?, ?) ORDER BY name")
...
export function updateMbAgentHeartbeat(id: string): void {
	const d = getDb();
	d.prepare(
		"UPDATE mb_agents SET last_heartbeat = ?, status = ? WHERE id = ?",
...
export function setMbAgentTask(
	id: string,
	task: string,
	taskPostId?: string,
): void {
	const d = getDb();
	d.prepare(
		"UPDATE mb_agents SET task = ?, task_post_id = ?, status = ? WHERE id = ?",
...
export function clearMbAgentTask(id: string): void {
	const d = getDb();
	d.prepare(
		"UPDATE mb_agents SET task = NULL, task_post_id = NULL, status = ? WHERE id = ?",
	).run("online", id);
}

// ─── MbLoop Operations ──────────────────────────────────────────────

export function createMbLoop(
	ownerAgent: string,
	goal: string,
	criteria: string,
...
export function getAllMbLoops(): MbLoop[] {
	const d = getDb();
	const rows = d
		.prepare("SELECT * FROM mb_loops ORDER BY start_time DESC")
		.all() as any[];
	return rows.map((r) => ({
		...r,
		agent_ids: parseJsonArray(r.agent_ids),
		consecutive_stuck: r.consecutive_stuck ?? 0,
		rescue_active: Boolean(r.rescue_active),
	}));
}

export function getActiveMbLoops(): MbLoop[] {
	return getAllMbLoops().filter(
		(loop) => loop.status === "running" || loop.status === "stuck",
	);
}
...
export function updateMbLoop(id: string, updates: Partial<MbLoop>): void {
	...
	if (updates.iteration !== undefined) {
		sets.push("iteration = ?");
		vals.push(updates.iteration);
	}
	if (updates.agent_ids !== undefined) {
		sets.push("agent_ids = ?");
		vals.push(JSON.stringify(updates.agent_ids));
	}
	if (updates.post_id !== undefined) {
		sets.push("post_id = ?");
		vals.push(updates.post_id);
	}
	if (updates.last_notice !== undefined) {
		sets.push("last_notice = ?");
		vals.push(updates.last_notice);
	}
	if (updates.check_command !== undefined) {
...
export function createTaskAssignment(
  ...
  `).run(taskPostId, assignedTo, assignedBy, now, now);
	return d
		.prepare("SELECT * FROM mb_task_assignments WHERE task_post_id = ?")
		.get(taskPostId) as MbTaskAssignment;
}

export function updateTaskAssignment(
	taskPostId: string,
	status: MbTaskAssignment["status"],
): void {
	const d = getDb();
	d.prepare(
		"UPDATE mb_task_assignments SET status = ?, updated_at = ? WHERE task_post_id = ?",
...
export function getAgentTasks(agentId: string): MbTaskAssignment[] {
	...
	return d
		.prepare(
			"SELECT * FROM mb_task_assignments WHERE assigned_to = ? ORDER BY created_at DESC",
		)
...
export function resetMbAll(): void {
	const d = getDb();
	d.exec(
		"DELETE FROM mb_task_assignments; DELETE FROM mb_loops; DELETE FROM mb_agents;",
	);
}
