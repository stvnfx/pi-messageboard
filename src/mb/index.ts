...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand('mb', {
    description: 'Messageboard agent management: /mb spawn, /mb status, /mb loop, /mb stop',
    handler: async (args, ctx) => {
      ...
      switch (subcommand) {
        case 'status': {
          ...
          if (loops.length > 0) {
            lines.push('', 'Loops:');
            lines.push(...loops.map(l => `  [${l.id.slice(0, 8)}] ${l.status} iter ${l.iteration}: ${l.goal.slice(0, 50)}`));
          }
