...
export function registerLoopTools(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      criteria: Type.Optional(Type.String({ description: 'Completion criteria' })),
      max_iterations: Type.Optional(Type.Number({ description: 'Max iterations (0=infinite)' })),
      model: Type.Optional(Type.String({ description: 'Model for agents' })),
      spawn_count: Type.Optional(Type.Number({ description: 'Number of agents to spawn (default 1)' })),
    }),
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      ctx.ui.notify(`🔄 Loop started: ${params.goal} (${agentIds.length} agents)`, 'info');
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (!loop) {
        return { content: [{ type: 'text', text: `Loop "${params.loop_id}" not found.` }], details: {}, isError: true };
      }
      ...
      boardDb.createMessage(
        myId,
        params.status === 'completed' ? 'resolved' : params.status === 'stuck' ? 'help' : 'info',
      ...
      return {
        content: [{ type: 'text', text: `Loop ${params.loop_id.slice(0, 8)} updated: ${params.status} (iter ${params.iteration})` }],
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      'Use mb_loop_stop to halt a running loop.',
    ],
    parameters: Type.Object({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      mbDb.updateMbLoop(params.loop_id, { status: 'paused', last_notice: 'Stopped by operator' });
      ...
      return {
        content: [{ type: 'text', text: `Loop ${params.loop_id.slice(0, 8)} stopped.` }],
...
export function getLoopDirective(kind: 'start' | 'continue' | 'stuck', loop: MbLoop): string {
  const iter = loop.iteration + 1;
  const label = loop.max_iterations > 0 ? `${iter}/${loop.max_iterations}` : `${iter}/∞`;
  ...
  switch (kind) {
    ...
    case 'continue':
      return CONTINUE_DIRECTIVES[Math.floor(Math.random() * CONTINUE_DIRECTIVES.length)] +
        `\nIteration: ${label}\nGoal: ${loop.goal}`;
    case 'stuck':
      return STUCK_STRATEGIES[Math.floor(Math.random() * STUCK_STRATEGIES.length)] +
        `\nIteration: ${label}\nGoal: ${loop.goal}\nCheck the board for help from other agents.`;
  }
}
