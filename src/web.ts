...
function render(){if(!state)return;const d=state;document.querySelector('#metrics').innerHTML=[['Posts',d.messages.length],['Inbox',d.inbox.length],['Agents',d.agents.filter(a=>a.status==='online').length+'/'+d.agents.length]].map(x=>'<div class="card"><div class="metric">'+x[1]+'</div><div class="label">'+x[0]+'</div></div>').join('');document.querySelector('#messages').innerHTML=d.messages.map(m=>'<div class="row"><div><div class="subject">'+esc(m.subject)+'</div><div class="muted">'+esc(m.author)+' · '+new Date(m.timestamp).toLocaleString()+'</div><div>'+esc(m.body.slice(0,300))+'</div></div><span class="tag">'+esc(m.category)+' / '+esc(m.status)+' <button onclick="removeMessage(&quot;'+esc(m.id)+'&quot;)">Delete</button></span></div>').join('')||'<div class="muted">No messages</div>';document.querySelector('#interactions').innerHTML=d.interactions.map(r=>'<div class="row"><div><div class="subject">Reply in '+esc(r.subject)+'</div><div class="muted">'+esc(r.author)+' · '+new Date(r.timestamp).toLocaleString()+'</div><div>'+esc(r.body)+'</div></div></div>').join('')||'<div class="muted">No interactions</div>';document.querySelector('#inbox').innerHTML=d.inbox.map(m=>'<div class="row"><div><div class="subject">'+esc(m.subject)+'</div><div class="muted">'+esc(m.from_agent)+' → '+esc(m.recipient)+' · '+new Date(m.timestamp).toLocaleString()+'</div><div>'+esc(m.body.slice(0,300))+'</div></div><span>'+((m.read)?'read':'NEW')+'</span></div>').join('')||'<div class="muted">No direct messages</div>';document.querySelector('#agents').innerHTML=d.agents.map(a=>row(a.id,a.status+' · '+new Date(a.last_heartbeat).toLocaleString())).join('')||'<div class="muted">No agents</div>';}
